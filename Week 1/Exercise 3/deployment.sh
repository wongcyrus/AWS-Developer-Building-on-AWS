...
VPC=$(aws cloudformation describe-stacks --stack-name edx-vpc-stack --query 'Stacks[0].Outputs[?OutputKey==`VPC`].OutputValue' --output text)
PublicSubnet1=$(aws cloudformation describe-stacks --stack-name edx-vpc-stack --query 'Stacks[0].Outputs[?OutputKey==`PublicSubnet1`].OutputValue' --output text)
